...
def Model_RAN(train_data, train_target, test_data, test_target, HN=None, BS=None, sol=None):
    if BS is None:
        BS = 4
    if sol is None:
        sol = [5, 5]
    ...
    STEP_SIZE_TRAIN = len(train_data) // BS
    ...
    with tf.device('/gpu:0'):
        ...
        model.summary()
        try:
            history = model.fit(X_train, steps_per_epoch=50, verbose=0, callbacks=callbacks,
                                batch_size=BS,
                                epochs=10, use_multiprocessing=True, workers=40)
        except:
            pass
        score = model.predict(X_test)
        score[score >= 0.5] = 1
        score[score < 0.5] = 0
        Eval = ClassificationEvaluation(test_target, score)
    return Eval, score
